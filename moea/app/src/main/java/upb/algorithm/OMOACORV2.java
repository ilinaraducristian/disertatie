...
// version 5
public class OMOACORV2 extends OMOPSO {
    ...
    public OMOACORV2(Problem problem) {
        this(
                problem,
                100,
                100,
    ...
    public OMOACORV2(Problem problem, int swarmSize, int leaderSize, double[] epsilons, double mutationProbability, double mutationPerturbation, int maxIterations, float vitezaDeConvergenta, int numarFurnici) {
    ...
    private void calculeazaDeviatiileStandard(int index) {
        ...
        Solution leaderSolution = leaders.get(rand.nextInt(leaders.size()));
        for (int i = 0; i < numberOfVariables; i++) {
            double leader = EncodingUtils.getReal(leaderSolution.getVariable(i));
