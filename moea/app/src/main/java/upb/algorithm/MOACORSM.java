...
// version 2
public class MOACORSM extends AbstractEvolutionaryAlgorithm implements EpsilonBoxEvolutionaryAlgorithm {
    ...
    public void iterate() {
        ...
        MOACORUtils MOACORUtils = new MOACORUtils(problem, properties, population, getVariation());
    ...
    private static class MOACORUtils {
        ...
        private void calculeazaPonderiSiProbabilitatiDeSelectie() {
            ...
            for (int i = 0; i < populationSize; i++) {
                ponderiW[i] = Math.exp((-1) * Math.pow(i, 2) / (2 * Math.pow(properties.getDouble("localitateaProcesuluiDeCautare", 1e-3), 2) * Math.pow(populationSize, 2))) / constanta;
            }
